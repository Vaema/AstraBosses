...
public abstract class NPCBehaviorOverride : ModType<NPC, NPCBehaviorOverride>
{
    ...
    /// <summary>
    ///     Allows you to prevent an NPC from doing anything on death (besides actually dying).
    ///     Return false to stop the NPC from doing anything special. Returns true by default.
    /// </summary>
    public virtual bool PreKill() => true;

    /// <summary>
    ///     Whether or not an NPC should be killed when it reaches zero health.
    ///     You may program extra effects in this hook (for example, how Golem's head lifts up for the second phase of its fight). Return false to stop the NPC from being killed. Returns true by default.
    /// </summary>
